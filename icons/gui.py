...
default_preferences = {'Server IP': 'myowncloudserver.com', 'Server Port': 443, 'Encryption Key': 91827, 'Signature': random.randint(1000, 9999)}

def loadPreferences():
    if os.path.exists(preferences_file):
        with open(preferences_file, 'r') as file:
            preferences = json.load(file)
            if not "Server IP" in preferences or not "Server Port" in preferences or not "Encryption Key" in preferences or not 'Signature' in preferences:
...
SIGNATURE = int(preferences['Signature'])
...
def run_process():
    if len(sys.argv) > 1:
        ...
        if os.path.isfile(file_path):
            ...
            status = send_file({"IP": IP, "Port": SERVER_PORT}, UUID, file_path, is_permanent_value, ENCRYPTION_KEY, SIGNATURE)
...
def receive_process():
    ...
    status = receive_file({"IP": IP, "Port": SERVER_PORT}, UUID, file_path, ENCRYPTION_KEY, SIGNATURE)
...
def settings_screen(event):
    ...
    # Signature input
    signature_label = ctk.CTkLabel(settings_screen, text="Signature:", font=("Arial", 16))
    signature_label.place(relx=0.5, y=390, anchor="center")

    signature_entry = ctk.CTkEntry(settings_screen)
    SIGNATURE = int(preferences['Signature'])
    signature_entry.insert(0, str(SIGNATURE))
    signature_entry.place(relx=0.5, y=420, anchor="center")

    save_signature_button = ctk.CTkButton(settings_screen, text="Save", command=lambda: save_signature(signature_entry.get()))
    save_signature_button.place(relx=0.5, y=460, anchor="center")
    ...
    def save_signature(signature):
        if signature.isnumeric():
            setPreference("Signature", signature)
